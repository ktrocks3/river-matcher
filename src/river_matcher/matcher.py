...
class MatchTiming:
    ...
    def measured_pipeline_seconds(self) -> float:
        return (
            self.arc_consistency_seconds
            + self.feasibility_dp_seconds
            + self.cost_setup_seconds
            + self.cost_dp_seconds
...
def _candidate_statistics(candidate_sets: NormalizedCandidateSets) -> CandidateStatistics:
    ...
    return CandidateStatistics(
        source_vertices=len(sizes),
        empty_domains=sum(size == 0 for size in sizes),
    ...
    *,
    ...
    for edge_index, edge in enumerate(sorted(source.edges, key=lambda item: item.id)):
        ...
        if not math.isfinite(value):
            raise RuntimeError(
                f"Recovered solution contains nonfinite cost for source edge e{edge.id}: "
        ...
        if witness is None:
            raise RuntimeError(
                f"Recovered solution has no witness for source edge e{edge.id}: "
                f"({edge.u}, {edge.v}) -> ({target_u}, {target_v})."
            )
    ...
    if not math.isclose(realized_value, solution.value, rel_tol=1e-10, abs_tol=1e-12):
        raise RuntimeError(
            f"Recovered {solution.objective.value} solution value is inconsistent: "
...
class RiverGraphMatcher:
    ...
    __slots__ = (
        ...
        "source",
        "target",
        "_arc_consistency_seconds",
        "_compatibility",
        "_compatibility_statistics",
        "_costs",
        "_effective_candidate_sets",
        "_effective_preflight",
        "_feasibility_dp_seconds",
        "_feasibility_statistics",
        "_feasible",
        "_lock",
    )
    ...
    def __init__(
        ...
        *,
        candidate_rho: float = 10.0,
        top_k: int = 25,
        candidate_sets: RawCandidateSets | None = None,
        decomposition: SourceDecomposition | None = None,
        validate_decomposition: bool = True,
    ) -> None:
        self.source = source
        self.target = target
        raw_candidate_sets = (
            compute_candidate_sets(source, target, rho=candidate_rho, top_k=top_k)
    ...
    def prepare_feasibility(
        ...
        *,
        ...
        with self._lock:
            ...
            effective, compatibility_statistics = enforce_arc_consistency(
                self._compatibility,
                self.candidate_sets,
            ...
            if effective_preflight.empty_domains:
                ...
                started = time.perf_counter()
                feasibility = solve_tree_feasibility(
                    self.decomposition,
                    effective,
                    self._compatibility,
                    cancellation_token=cancellation_token,
    ...
    def _common_metadata(self) -> dict[str, object]:
        return {
            ...
            "candidate_statistics": self.candidate_statistics,
            "decomposition": self.decomposition,
            "effective_candidate_sets": self.effective_candidate_sets,
            "effective_candidate_statistics": self.effective_candidate_statistics,
            "preflight": self.preflight,
            "effective_preflight": self.effective_preflight,
            "compatibility_statistics": self._compatibility_statistics,
    ...
    def _feasibility_timing(self, arc_before: float, feasibility_before: float, reused: bool) -> MatchTiming:
        return MatchTiming(
            ...
            feasibility_dp_seconds=self._feasibility_dp_seconds - feasibility_before,
        ...
        **cost_options: object,
        ...
        with self._lock:
            ...
            candidate_violations = tuple(
                ...
                if target_vertex not in self.candidate_sets[source_vertex]
            )
            effective_candidate_violations = tuple(
                source_vertex
                for source_vertex, target_vertex in normalized.items()
                if target_vertex not in self.effective_candidate_sets[source_vertex]
            )
            timing = MatchTiming(
                cost_setup_seconds=cost_setup_seconds,
                materialization_seconds=evaluation_seconds,
    ...
    def match(
        ...
        **cost_options: object,
        ...
        started = time.perf_counter()
        solution = _materialize_solution(
            self.source,
            edge_cost,
            result.solution,
            cancellation_token=cancellation_token,
    ...
    def match_both(
        ...
        **cost_options: object,
        ...
        cost_dp_seconds = time.perf_counter() - started
        started = time.perf_counter()
        additive = _materialize_solution(
            self.source,
            edge_cost,
            result.additive,
            cancellation_token=cancellation_token,
        )
        bottleneck = _materialize_solution(
            self.source,
            edge_cost,
            result.bottleneck,
            cancellation_token=cancellation_token,
