...
class PairSession:
    ...
    def match(self, cost_name: str, options: Mapping[str, object]) -> tuple[BothMatchResult, bool, float]:
        ...
        with self._lock:
            ...
            result = self.matcher.match_both(cost_name, **dict(options))
...
class MatchWorker(QRunnable):
    ...
    def run(self) -> None:
        try:
            self.signals.progress.emit("Loading graphs")
            first = self.repository.load(self.first_path)
            second = self.repository.load(self.second_path)
            source, target, swapped = normalize_sparse_to_dense(first, second)

            self.signals.progress.emit("Preparing sparse-to-dense matching session")
            ...
            for index, (cost_name, options) in enumerate(self.costs, start=1):
                self.signals.progress.emit(f"Computing {cost_name.replace('_', ' ')} ({index}/{len(self.costs)})")
                result, from_cache, elapsed = session.match(cost_name, options)
                self.signals.result.emit(
                    RunOutcome(
                        source_path=source.key.path,
                        target_path=target.key.path,
