...
def make_graphs() -> tuple[JunctionGraph, JunctionGraph]:
    ...
    target = JunctionGraph(
        name="target",
        coordinates={10: (0.0, 0.0), 20: (1.0, 1.0), 30: (2.0, 0.0), 40: (0.0, 1.0), 50: (2.0, 1.0), 60: (10.0, 0.0), 70: (11.0, 0.0)},
        edges=(
            make_edge(10, 10, 20, [(0.0, 0.0), (1.0, 1.0)]),
            make_edge(11, 20, 30, [(1.0, 1.0), (2.0, 0.0)]),
            make_edge(12, 40, 50, [(0.0, 1.0), (2.0, 1.0)]),
...
def dynamic_time_warping_reference(first: FloatArray, second: FloatArray) -> float:
    """
    Independent unrestricted DTW dynamic program.

    Local costs are squared Euclidean distances. The final square root matches
    DTAIDistance's ``squared Euclidean`` result transformation.
    """
    table = np.full((len(first) + 1, len(second) + 1), np.inf, dtype=np.float64)
    table[0, 0] = 0.0
...
def test_factory_creates_dynamic_time_warping_cost() -> None:
    cost = make_cost()

    assert cost.name is CostName.DYNAMIC_TIME_WARPING_DISTANCE
    assert cost.label == "normalized dynamic time warping distance"
...
def test_parallel_lines_have_unit_normalized_distance() -> None:
    cost = make_cost(curve_samples=33)
...
def test_disabling_normalization_returns_raw_dtw_distance() -> None:
    curve_samples = 33
    cost = make_cost(curve_samples=curve_samples, normalize=False)

    value = cost(0, 1, 2, 40, 50)

    assert value == pytest.approx(math.sqrt(curve_samples))


def test_window_one_uses_diagonal_matching_for_equal_lengths() -> None:
    cost = make_cost(curve_samples=33, warping_window=1, normalize=False)

    value = cost(0, 1, 2, 40, 50)

    assert value == pytest.approx(math.sqrt(33.0))


def test_unrestricted_window_is_supported() -> None:
    cost = make_cost(warping_window=None)

    assert cost.warping_window is None
    assert cost(1, 1, 2, 10, 30) == pytest.approx(0.0)
...
def test_source_sample_cache_stores_both_orientations() -> None:
    ...
    assert reverse.flags.writeable
    assert reverse.flags.c_contiguous
    assert reverse.dtype == np.float64
...
@pytest.mark.parametrize("warping_window", [0, -1, -10])
def test_invalid_warping_window_is_rejected(warping_window: int) -> None:
    with pytest.raises(ValueError, match="DTW warping window must be at least 1 or None"):
        make_cost(warping_window=warping_window)
...
def test_factory_reuses_guided_finder_for_matching_options() -> None:
    ...
    first = factory.create("dynamic_time_warping_distance", rho=2.0, edge_samples=8, curve_samples=16, warping_window=4)
    second = factory.create("dynamic_time_warping_distance", rho=2.0, edge_samples=8, curve_samples=64, warping_window=None, normalize=False)

    assert isinstance(first, DynamicTimeWarpingDistance)
    assert isinstance(second, DynamicTimeWarpingDistance)
...
def test_factory_separates_guided_finders_with_different_options() -> None:
    ...
    first = factory.create("dynamic_time_warping_distance", rho=1.0, edge_samples=8)
    second = factory.create("dynamic_time_warping_distance", rho=2.0, edge_samples=8)
    third = factory.create("dynamic_time_warping_distance", rho=1.0, edge_samples=16)

    assert isinstance(first, DynamicTimeWarpingDistance)
    assert isinstance(second, DynamicTimeWarpingDistance)
    assert isinstance(third, DynamicTimeWarpingDistance)
...
def test_library_matches_independent_unrestricted_reference() -> None:
    ...
    for _ in range(40):
        ...
        expected = dynamic_time_warping_reference(first, second)
        actual = float(dtw_ndim.distance_fast(first, second, window=None, inner_dist="squared euclidean"))
